...
public class FordFulkerson {
    ...
    public static void main(String[] args) {
        if (args.length != 1) {
            System.err.println("Usage: java MaxFlowProgram filename.txt");
            return;
        }
    
        In in = new In(args[0]);              // Đọc dữ liệu từ file
        FlowNetwork G = new FlowNetwork(in);  // Khởi tạo mạng dòng từ file
    
        int s = 0;                // đỉnh nguồn
        int t = G.V() - 1;        // đỉnh đích (đỉnh cuối cùng)
